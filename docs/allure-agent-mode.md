...
- Main automated scope: JUnit 4 test classes under `src/test/java/io/qameta/allure/bamboo`, usually run with `./mvnw ... test`.
- Compatibility smoke scope: the `compat/bamboo-specs` harness, run with `./mvnw -f compat/bamboo-specs/pom.xml ... exec:java` after a plugin JAR exists in `target/`.
- Manual Bamboo investigation via `atlas-run` is useful for local debugging, but it is not a substitute for agent-mode evidence from the narrowest automated scope you can run.
...
1. Split audits by narrow scopes such as one JUnit class, one package area, or one compatibility target version.
2. Give each scope its own expectations file and temp output directory.
3. Run each scope with `allure run`.
4. Review runtime artifacts first, then inspect source code only after the run explains what actually executed.
5. Mark the review incomplete until each scope either matched expectations or was explicitly documented as a broad package-health audit.
...
Targeted JUnit review example:

```yaml
goal: Review a Bamboo plugin unit test
task_id: allure-executable-test
expected:
  label_values:
    package: io.qameta.allure.bamboo
notes:
  - Prefer a single test class or other narrow scope when possible.
  - If the run lacks stable label metadata, call that out instead of inventing labels.
```

Compatibility smoke example:

```yaml
goal: Review Bamboo compatibility smoke
task_id: bamboo-compat-smoke
expected:
  label_values:
    framework: compat
notes:
  - Review the generated compatibility summary and attached runtime evidence before source inspection.
```

### Command Patterns

Target one JUnit class:
...
allure run -- ./mvnw -Dtest=AllureExecutableTest test
```
...
ALLURE_AGENT_OUTPUT="$TMP_DIR/agent-output" \
allure run -- ./mvnw test
```

Run the compatibility smoke harness:
...
ALLURE_AGENT_OUTPUT="$TMP_DIR/agent-output" \
ALLURE_AGENT_EXPECTATIONS="$EXPECTATIONS" \
allure run -- ./mvnw -q -f compat/bamboo-specs/pom.xml \
  -Dcompat.rootDir="$(pwd)" \
...
If `allure run` is not available in your shell, install or expose the Allure CLI you use locally before relying on console-only results.
